...
def seed_minimum_data():
    ...
    upsert_scenario(
        id="scn_folk_01",
        name="전래동화 1 (예: 볏단 이야기)",
        story="여기에 전래동화 원문/요약을 넣습니다. (너무 길면 요약 권장)",
        goal="아이 스스로 형제애/배려 같은 보이지 않는 가치를 발견하도록 돕는다.",
        outline=(
            "형제가 서로를 위해 몰래 볏단을 옮기며 마음을 확인하는 이야기. "
            "누가 더 손해를 보느냐가 아니라, 왜 그렇게 했는지가 핵심."
        ),
        scenario_prompt=(
        ...
        first_message="안녕! 오늘은 재미있는 전래동화를 같이 생각해볼까?",
        is_active=True,
    )
    upsert_scenario(
        id="scn_folk_02",
        name="전래동화 2 (예: 흥부와 놀부)",
        story="여기에 전래동화 원문/요약을 넣습니다.",
        goal="선의/나눔의 의미와 결과를 감정과 판단 모두로 이해하도록 돕는다.",
        outline=(
            "어려운 상황에서도 나누는 선택이 어떤 마음과 결과를 만들었는지 살펴보는 이야기."
        ),
        scenario_prompt=(
            "진행 규칙: 1) 개요 -> 2) 목표 -> 3) 캐릭터 방식 진행. "
            "4) 아이 답을 먼저 확인하고, 다음 질문/공감으로 연결한다."
        ),
        first_message="안녕! 오늘 이야기를 같이 들여다보자.",
        is_active=True,
    )
